...
import plotly.graph_objects as go
    # Combine region files into a single unified view (no AU/NZ tabs)
    combined_events = []
    combined_flags = []
    ...
    display_data = (first_summary or {})
    # replace lists with combined lists
    display_data = dict(display_data)  # shallow copy
    display_data['events'] = combined_events
    display_data['featureFlags'] = combined_flags
    display_data['experiments'] = combined_experiments

    # Embed combined JSON for client-side export and search
    try:
        combined_json = json.dumps(display_data, ensure_ascii=False).replace('</', '<\\/')
    except Exception:
        combined_json = '{}'

    # Render unified optimizely UI with a single search box and export button
    return f"""
        <div class="optly-container" id="optly-container">
            ...
            <div class="optly-inner">
                {_optly_render_tab_content(display_data, 'combined')}
            </div>
        </div>
        <script>
            (function() {{
                ...
                // Collapsible groups and row expansion
                ...
                container.querySelectorAll('.optly-json-key.optly-has-children').forEach(key => {{
                    key.addEventListener('click', (e) => {{
                        ...
                        if (!node || !icon) return;
                        if (node.classList.contains('collapsed')) {{ node.classList.remove('collapsed'); icon.textContent = '▼'; }} else {{ node.classList.add('collapsed'); icon.textContent = '▶'; }}
                    }});
                }});

                // Search filtering (limited to visible/important fields to avoid JSON-only matches)
                ...
                const filterRows = () => {{
                    ...
                    rows.forEach(row => {{
                        ...
                        if (!term || haystack.indexOf(term) !== -1) row.classList.remove('hidden'); else row.classList.add('hidden');
                    }});
                    // Show no-results message per group
                    container.querySelectorAll('.optly-expandable-table').forEach(table => {{
                        ...
                        if (term && visibleRows.length === 0) {{ if (!message) {{ message = document.createElement('div'); message.className = 'optly-no-results'; message.textContent = 'No results found for "' + searchInput.value + '"'; table.appendChild(message); }} else {{ message.textContent = 'No results found for "' + searchInput.value + '"'; }} }} else if (message) {{ message.remove(); }}
                    }});
                }};
                ...
                if (clearButton) clearButton.addEventListener('click', () => {{ if (searchInput) {{ searchInput.value = ''; filterRows(); searchInput.focus(); }} }});

                // Export combined Optimizely data to CSV
                const exportBtn = document.getElementById('optly-export-csv');
                if (exportBtn) {{
...
def generate_category_hierarchy_visualization(df, region):
    ...
    # Ensure historical_json and dates_json are always defined
    historical_json = locals().get('historical_json', '{}')
    dates_json = locals().get('dates_json', '[]')
...
def _load_broken_rows(conn: sqlite3.Connection, target_date: date) -> list[dict]:
    """Load full broken link rows (as dicts) for a given historical date table.
    Returns an empty list if the table is missing or an error occurs.
    """
    table = f"broken_links_{target_date.strftime('%Y_%m_%d')}"
    cur = conn.cursor()
    try:
        cur.execute(f"SELECT Region, URL, Status, Response_Time, Error_Message, Timestamp FROM {table}")
        rows = cur.fetchall()
    except sqlite3.OperationalError:
        return []
    cols = ['Region', 'URL', 'Status', 'Response_Time', 'Error_Message', 'Timestamp']
...
def _compute_changes(conn: sqlite3.Connection):
    ...
    # Pick the most-recent previous date strictly before today for "yesterday" (if any)
    yday_d = None
    previous_dates = [d for d in available_dates if d < today_d]
    if previous_dates:
        # Choose the newest date that is still older than today
        yday_d = max(previous_dates)

    # Choose a date to represent "7 days ago" comparision.
    # Prefer the latest available date that is <= target_week_date so we compare against the closest
    # snapshot that is at least 7 days old. If there is no such table, weekly comparison will be disabled.
    week_d = None
    target_week_date = today_d - timedelta(days=7)
    week_candidates = [d for d in available_dates if d <= target_week_date]
    if week_candidates:
        week_d = max(week_candidates)
...
def _optly_render_group(title, items, region_key, item_type, open_by_default=False):
    ...
    # Limit search payload to visible fields only (key, id, name, region) to avoid matching nested JSON fields
    allowed_keys = ['key', 'id', 'name', 'region']
        ...
        # Append region label to the visible title if present (helpful in combined view)
        try:
            region_label = item.get('region') if isinstance(item, dict) else None
        except Exception:
            region_label = None
        if region_label:
            row_title = f"{row_title} [{html.escape(str(region_label).upper())}]"
...
def generate_combined_html_report(au_csv_path, nz_csv_path, output_html_path='combined_report.html', product_csv_path='product_export.csv', db_path='broken_links.db', optimizely_json_path='kmart.json'):
    ...
    try:
        ...
        # Build historical rows for the last 7 available dates (including today) for client side
        hist_dates = changes.get('available_dates', [])[:7]
        historical_data = {}
        for d in hist_dates:
            rows = _load_broken_rows(conn, d)
            # Normalize row values for safe JSON serialization
            normalized = []
            for r in rows:
                try:
                    status_val = r.get('Status', '')
                    status_val = int(status_val) if str(status_val).isdigit() else status_val
                except Exception:
                    status_val = r.get('Status', '')
                normalized.append({
                    'Region': r.get('Region', ''),
            ...
            historical_data[d.strftime('%Y-%m-%d')] = normalized
        # Prepare JSON blobs for client-side rendering (escaped to avoid closing script tags)
        try:
            hist_date_strs = [d.strftime('%Y-%m-%d') for d in hist_dates]
            historical_json = json.dumps(historical_data, ensure_ascii=False).replace('</', '<\\/')
            dates_json = json.dumps(hist_date_strs)
        except Exception:
            historical_json = '{}'
            dates_json = '[]'
    ...
    def render_changes_section(title: str, items: list[tuple[str,str]], change_type: str, table_id: str = None):
        # items: list of (Region, URL)
        if not items:
            return "<p>No changes.</p>"
        
        # Create unique table ID based on title and change type if not provided
        if not table_id:
        ...
        yesterday_content = f"""
                    ...
                    <div class="collapsible-section">
                        ...
                        <div class="collapsible-content" id="yesterday-added-section">
                            {render_changes_section('Yesterday Added', changes['yesterday']['added'], 'added', table_id='yesterday-added-table')}
                        </div>
                    </div>
                    <div class="collapsible-section">
                        ...
                        <div class="collapsible-content" id="yesterday-removed-section">
                            {render_changes_section('Yesterday Removed', changes['yesterday']['removed'], 'removed', table_id='yesterday-removed-table')}
                        </div>
                    </div>
        """
    else:
        ...
        week_content = f"""
                    ...
                    <div class="collapsible-section">
                        ...
                        <div class="collapsible-content" id="week-added-section">
                            {render_changes_section('Added Week', changes['week']['added'], 'added', table_id='week-added-table')}
                        </div>
                    </div>
                    ...
                    <div class="collapsible-section">
                        ...
                        <div class="collapsible-content" id="week-removed-section">
                            {render_changes_section('Removed Week', changes['week']['removed'], 'removed', table_id='week-removed-table')}
                        </div>
                    </div>
        """
    else:
    ...
    changes_html = f"""
        <div id="Changes" class="tab-content">
            <div class="card" style="margin-bottom: 12px;">
                <label for="changes-date-picker" style="font-weight:600; margin-right: 8px;">Select Date:</label>
                <input type="text" id="changes-date-picker" name="changes-date-picker" readonly aria-label="Select snapshot date" />
                <button id="show-date-btn" style="margin-left:8px; padding:8px 12px;">Show</button>
                <span style="margin-left: 12px; color: #6c757d; font-size: 13px;">(Choose a date from the last 7 snapshots)</span>
            </div>
            <div id="selected-date-details" class="card" style="display:none; margin-bottom: 12px;"></div>
            <div id="seven-day-summary" class="card" style="margin-bottom: 12px;">
...
{week_content}
    ...
    """
    # NOTE: historical JSON will be embedded directly into the final HTML head below
    ...
    <script>window.__HISTORICAL_DATA = {historical_json}; window.__HISTORICAL_DATES = {dates_json};</script>
    ...
    <body>
        ...
        <header>
            <div style="display:flex; align-items:center; max-width: var(--container-max); margin: 0 auto; gap: 12px;">
        ...
        <script>
            ...
            $(document).ready(function() {{
                ...
                // Render historical data based UI for Changes tab
                try {
                    var hist = window.__HISTORICAL_DATA || {};
                    var histDates = window.__HISTORICAL_DATES || [];

                    if (histDates && histDates.length > 0) {
                        // Populate date picker with the available dates — prefer flatpickr which can enable specific dates
                        var datePicker = document.getElementById('changes-date-picker');
                        if (datePicker) {
                            if (typeof flatpickr !== 'undefined') {
                                try {
                                    flatpickr(datePicker, {
                                        dateFormat: 'Y-m-d',
                                        enable: histDates,
                                        defaultDate: histDates[0] || null,
                                        disableMobile: true
                                    });
                                } catch (err) {
                                    console.warn('flatpickr initialization failed, falling back to native date input', err);
                                    datePicker.type = 'date';
                                    datePicker.min = histDates[histDates.length - 1];
                                    datePicker.max = histDates[0];
                                    datePicker.value = histDates[0];
                                    datePicker.addEventListener('change', function() {
                                        if (histDates.indexOf(this.value) === -1) {
                                            alert('Selected date is not available. Please pick an available snapshot.');
                                            this.value = histDates[0];
                                        }
                                    });
                                }
                            } else {
                                // No flatpickr: use native date input but validate selection
                                datePicker.type = 'date';
                                datePicker.min = histDates[histDates.length - 1];
                                datePicker.max = histDates[0];
                                datePicker.value = histDates[0];
                        ...
                        // Build seven-day summary table showing added/removed relative to the previous snapshot
                        var summaryContainer = document.getElementById('seven-day-summary-table');
                        if (summaryContainer) {
                            var rows = [];
                            ...
                            for (var i = 0; i < histDates.length; i++) {
                                var d = histDates[i];
                                var current = hist[d] || [];
                                var prev = (i + 1 < histDates.length) ? (hist[histDates[i + 1]] || []) : [];

                                // Build sets keyed by Region + separator + URL for comparisons
                                var sep = '|||';
                                var currSet = new Set();
                                var prevSet = new Set();
                                current.forEach(function(it) { currSet.add((it.Region || '') + sep + (it.URL || '')); });
                                ...
                                var addedAU = 0, addedNZ = 0, removedAU = 0, removedNZ = 0;
                                added.forEach(function(k) { var parts = k.split(sep); var region = (parts[0] || '').toUpperCase(); if (region === 'AU') addedAU++; else if (region === 'NZ') addedNZ++; });
                                removed.forEach(function(k) { var parts = k.split(sep); var region = (parts[0] || '').toUpperCase(); if (region === 'AU') removedAU++; else if (region === 'NZ') removedNZ++; });

                                // Summary row
                                rows.push('<tr class="summary-row" data-date="' + d + '"><td>' + d + '</td><td>' + addedAU + '</td><td>' + removedAU + '</td><td>' + addedNZ + '</td><td>' + removedNZ + '</td><td><button class="details-btn" data-date="' + d + '">Toggle</button></td></tr>');

                                // Hidden details row for expand/collapse
                                rows.push('<tr id="summary-' + d + '-details" class="summary-details" style="display:none;"><td colspan="6"></td></tr>');
                            }

                            rows.push('</tbody></table>');
                            summaryContainer.innerHTML = rows.join('');

                            // Attach handlers for detail buttons to toggle inline diffs
                            summaryContainer.querySelectorAll('.details-btn').forEach(function(btn) {
                                btn.addEventListener('click', function() {
                                    var dt = this.getAttribute('data-date');
                                    var detailsRow = document.getElementById('summary-' + dt + '-details');
                                    if (!detailsRow) return;
                                    if (detailsRow.style.display === 'none' || detailsRow.style.display === '') {
                                        // Render added/removed lists
                                        var idx = histDates.indexOf(dt);
                                        var prevDate = (idx + 1 < histDates.length) ? histDates[idx + 1] : null;
                                        var curr = hist[dt] || [];
                                        var prev = prevDate ? (hist[prevDate] || []) : [];

                                        var sepLocal = '|||';
                                        var currSetLocal = new Set();
                                        var prevSetLocal = new Set();
                                        curr.forEach(function(it) { currSetLocal.add((it.Region || '') + sepLocal + (it.URL || '')); });
                                        prev.forEach(function(it) { prevSetLocal.add((it.Region || '') + sepLocal + (it.URL || '')); });

                                        var addedList = [];
                                        var removedList = [];
                                        currSetLocal.forEach(function(k) { if (!prevSetLocal.has(k)) addedList.push(k); });
                                        prevSetLocal.forEach(function(k) { if (!currSetLocal.has(k)) removedList.push(k); });

                                        function renderList(arr) {
                                            if (!arr || arr.length === 0) return '<div style="padding:10px;color:#6c757d;">No items</div>';
                                            var html = '<ul style="margin:8px 0;padding-left:16px;">';
                                            arr.forEach(function(k) { var p = k.split(sepLocal); var region = htmlEscape(p[0] || ''); var url = htmlEscape(p.slice(1).join(sepLocal) || ''); html += '<li><strong>' + region + '</strong>: <a href="' + url + '" target="_blank">' + url + '</a></li>'; });
                                            html += '</ul>';
                                            return html;
                                        }

                                        var detailHtml = '<div style="display:flex;gap:18px;flex-wrap:wrap;">';
                                        detailHtml += '<div style="flex:1;min-width:280px;"><h4 style="margin-top:0;">Added</h4>' + renderList(addedList) + '</div>';
                                        detailHtml += '<div style="flex:1;min-width:280px;"><h4 style="margin-top:0;">Removed</h4>' + renderList(removedList) + '</div>';
                                        detailHtml += '</div>';

                                        detailsRow.cells[0].innerHTML = detailHtml;
                        ...
                        // No historical dates
                        var summaryContainer = document.getElementById('seven-day-summary-table');
                        if (summaryContainer) summaryContainer.innerHTML = '<div style="padding:12px; color:#6c757d;">No historical snapshots available.</div>';
                    }
                    ...
                    function renderSelectedDate(dateStr) {
                        ...
                        if (!data || data.length === 0) {
                            ...
                            target.innerHTML = '<div style="padding:12px; color:#6c757d;">No data for ' + dateStr + '.</div>';
                            return;
                        }
                        var htmlRows = [];
                        htmlRows.push('<h4 style="margin-top:0;">Snapshot: ' + dateStr + ' (' + data.length + ' broken links)</h4>');
                        htmlRows.push('<table style="width:100%"><thead><tr><th>Region</th><th>Status</th><th>URL</th><th>Error Message</th><th>Timestamp</th></tr></thead><tbody>');
                        data.forEach(function(r) {
                            var status = r.Status || '';
                            var cls = (status >= 400) ? 'error' : (status >= 300) ? 'redirect' : 'ok';
                            htmlRows.push('<tr class="'+cls+'"><td>'+htmlEscape(r.Region || '')+'</td><td>'+htmlEscape(''+status)+'</td><td><a href="'+htmlEscape(r.URL || '')+'" target="_blank">'+htmlEscape(r.URL || '')+'</a></td><td>'+htmlEscape(r.Error_Message || '')+'</td><td>'+htmlEscape(r.Timestamp || '')+'</td></tr>');
                        });
                        htmlRows.push('</tbody></table>');
                        target.style.display = 'block';
                        target.innerHTML = htmlRows.join('');
                        // Initialize DataTable on the rendered table for filtering and paging
                        try {
                            $('#selected-date-details table').DataTable({ pageLength: 50 });
                        } catch (e) { /* ignore if DataTables not present */ }
                    }

                    function htmlEscape(str) {
                        return (str || '').toString().replace(/[&<>"']/g, function(ch) { return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":"&#39;"}[ch]; });
                    }
                } catch (e) {
                    console.log('Error rendering historical UI: ', e);
                }
            }});
